...
function weierstrass(curveDef) {
    ...
    function verify(signature, msgHash, publicKey, opts = defaultVerOpts) {
        ...
        let P;
        try {
            if (typeof sg === 'string' || isBytes$3(sg)) {
...
var cipher = /*#__PURE__*/Object.freeze({
    ...
    decrypt_gcm: decrypt_gcm,
    encrypt_cbc: encrypt_cbc,
    encrypt_gcm: encrypt_gcm
});
...
function pkdf256(secret, salt, count = 2048) {
    ...
    const key = pbkdf2(sha256$1, sec, slt, { c: count, dkLen: 32 });
    return Buff.raw(key);
}
function pkdf512(secret, salt, count = 2048) {
    const sec = Buff.bytes(secret);
    const slt = Buff.bytes(salt);
    const key = pbkdf2(sha512$1, sec, slt, { c: count, dkLen: 64 });
    return Buff.raw(key);
}
...
var hd = /*#__PURE__*/Object.freeze({
    ...
    derive_pubkey: derive_pubkey,
    derive_seckey: derive_seckey,
...
export { _const as CONST, Field, Point, assert, cipher, hash, hd, keys, math, merkle, noble, shamir, sig as signer, util };
