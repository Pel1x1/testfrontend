...
import { get_pubkey, get_seckey, parse_pubkey } from './keys.js';
export function ecdh(self_sec, peer_pub, xonly = false) {
    const P = Point.from_x(peer_pub);
    const sec = get_seckey(self_sec);
    const shared = P.mul(sec);
    return (xonly) ? shared.x : shared.buff;
}
export function ecdhash(self_sec, peer_pub, xonly = false) {
